...
func (repo *ProductRepository) GetAll() ([]models.Product, error) {
	query := "SELECT id, name, \"desc\", price, stock, category_id, created_at FROM products"
	...
	for rows.Next() {
		...
		err := rows.Scan(&p.ID, &p.Name, &p.Desc, &p.Price, &p.Stock, &categoryID, &p.CreatedAt)
		if err != nil {
			return nil, err
		}
...
func (repo *ProductRepository) GetByID(id int) (*models.Product, error) {
	query := "SELECT id, name, \"desc\", price, stock, category_id, created_at FROM products WHERE id = $1"

	var p models.Product
	var categoryID sql.NullInt64
	err := repo.db.QueryRow(query, id).Scan(&p.ID, &p.Name, &p.Desc, &p.Price, &p.Stock, &categoryID, &p.CreatedAt)
